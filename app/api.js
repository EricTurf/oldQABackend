...
exports.addQuestion = function(req, res) {
  ...
  if (obj[0].question == null || obj[0].question == '') {
    res.json({
      success: false,
      message: "One of the question fields was left blank. Please insert a question(s)"
    });
  } else {
...
exports.compileAnswers = function(req, res) {
  userData(req.params.refID, function(err, email) {
    if (err) {
      res.json({
        success: false,
        message: 'Could not retrieve the proper e-mail assosiated to this interview!'
      });
    } else if (email) {

      var obj = (req.body);
      var subject = 'Interview conducted by ' + req.params.fname + ' ' + req.params.lname;
...
exports.createUser = function(req, res) {
  ...
  if (req.body.email == null || req.body.email == '' || req.body.password == null || req.body.password == '') {
    ...
    var query = "INSERT INTO ??(??,??) VALUES (?,?)";
    var table = ["users", "email", "password", req.body.email, req.body.password];
    query = mysql.format(query, table);
    connection.query(query, function(err, rows) {
      if (err) {
        res.json({
        ...
        if (tempToken) {
          var subject = 'Activation link for your QuestionApp account: ' + req.body.email;
          var msg = 'To activate your account, please click on the following link: https://radiant-lake-20483.herokuapp.com/activateUser/' + tempToken;
